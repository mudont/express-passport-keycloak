...
router.post("/", function (req, res, next) {
    ...
    db.run("INSERT INTO todos (owner_id, title, completed) VALUES (?, ?, ?)", [req.user.id, req.body.title, req.body.completed == true ? 1 : null], function (err) {
        if (err) {
            return next(err);
        }
        return res.redirect("/" + (req.body.filter || ""));
...
router.post("/:id(\\d+)", function (req, res, next) {
    ...
    db.run("DELETE FROM todos WHERE id = ? AND owner_id = ?", [req.params.id, req.user.id], function (err) {
        if (err) {
            return next(err);
        }
        return res.redirect("/" + (req.body.filter || ""));
...
router.post("/toggle-all", function (req, res, next) {
    db.run("UPDATE todos SET completed = ? WHERE owner_id = ?", [req.body.completed !== undefined ? 1 : null, req.user.id], function (err) {
        if (err) {
            return next(err);
        }
        return res.redirect("/" + (req.body.filter || ""));
